...
Modern apps rely on background processing to deliver great user experiences. When someone places an order, your system handles payments, updates inventory, sends emails, and tracks analytics—all without making the user wait. The issue is that building reliable async workflows is tough. Networks fail, APIs flake out, and one missed step can mean lost revenue or frustrated users.
...
> [great spot for a 1 min concept video]
...
It then **calls your code** (through HTTP requests) when it's time to run, and finally **makes sure functions complete** with retries and state tracking.
...
Before exploring the key concepts that are making this approach possible, it's time to use our brain (as an analogy) to understand what makes Inngest so neat, and more importantly what it'll empower you to do.
...
**Your app** can send events using the Inngest SDK when important things happen. **Webhooks**, to receive events from services like Stripe, GitHub, or Shopify. **Schedules** can trigger events on a schedule, like cron jobs. **Other functions** to chain workflows by having functions send events.

**Events are handy to manipulate.**   
**Filter** them to only run functions when events match certain rules, **batch** similar events to process them together, or **transform** them by changing or adding data as they flow through.
...
Those three pillars are the foundation of Inngest, but what makes it truly powerful is the developer experience. Inngest is designed to get you up and running, **fast**.
...
- [**Deploy to prod**](https://www.inngest.com/docs/platform/deployment): Learn how Inngest works under the hood
